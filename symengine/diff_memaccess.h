...
class DiffVisitor2 : public BaseVisitor<DiffVisitor2>
{
    ...
    using DetailedExprs
        = std::vector<std::pair<RCP<const Basic>, VecMatchesMap>>;
...
public:
    DiffVisitor2(bool resolveModulos, bool debug = false)
        : m_bResolveModulos(resolveModulos), m_bDebug(debug)
    {
    }

    RCP<const Basic> Apply(RCP<const Basic> &expr, RCP<const Basic> wrt);
...
private:
    void
    DynamicLoop(const std::vector<size_t> &lows,
    ...
    __always_inline void PrintDetailedExprs(const DetailedExprs &de,
                                            const std::string &msg);
    __always_inline DetailedExprs
    AggrigateTwoDetailedExprs(const DetailedExprs &de1,
                              const DetailedExprs &de2, bool isAdd, bool isMul);
};